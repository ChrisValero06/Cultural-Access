...
const CargarPromoFunctional = () => {
  ...
  const handleChange = (e) => {
    ...
    if (files && files[0]) {
      ...
      // Al cambiar institución, verificar si el tipo actual sigue siendo válido
      const tipoActualValido = todosLosTipos.some(t => {
        const nombre = t.nombre || t;
        const instituciones = t.instituciones || [];
        return nombre === formData.tipoPromocion && (instituciones.length === 0 || instituciones.includes(value));
      });
      setFormData(prev => ({ ...prev, institucion: value, tipoPromocion: tipoActualValido ? prev.tipoPromocion : '' }))
    } else {
  ...
  useEffect(() => {
    const institucionSeleccionada = formData.institucion;
