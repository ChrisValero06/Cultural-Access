...
const CargarPromo = () => {
  ...
  const [autocompleteStates, setAutocompleteStates] = useState({
    institucion: { show: false, filtered: [] },
    tipoPromocion: { show: false, filtered: [] },
    disciplina: { show: false, filtered: [] }
  })
  ...
  const refs = { institucion: useRef(null), tipoPromocion: useRef(null), disciplina: useRef(null) }
    ...
    const handleClickOutside = (event) => {
      Object.entries(refs).forEach(([key, ref]) => {
        if (ref.current && !ref.current.contains(event.target)) {
          setAutocompleteStates(prev => ({ ...prev, [key]: { show: false, filtered: [] } }))
        }
      })
    }
  ...
  const updateAutocomplete = (field, value) => {
    const data = DATA[field]
    const filtered = value.trim() === '' ? [] : data.filter(item => item.toLowerCase().includes(value.toLowerCase()))
    setAutocompleteStates(prev => ({ ...prev, [field]: { show: filtered.length > 0, filtered } }))
  }
  ...
  const handleChange = (e) => {
    ...
    if (files) {
      ...
      if (['institucion', 'tipoPromocion', 'disciplina'].includes(name)) updateAutocomplete(name, value)
    }
  }

  const selectOption = (field, value) => {
    setFormData(prev => ({ ...prev, [field]: value }))
    setAutocompleteStates(prev => ({ ...prev, [field]: { show: true, filtered: DATA[field] } }))
  }
  ...
  const AutocompleteField = React.forwardRef(({ name, label, placeholder, value, onChange, onFocus }, ref) => {
    const state = autocompleteStates[name]
    return (
      <div>
        <label htmlFor={name} className="block text-base font-bold text-white mb-2">{label}*</label>
        <div className="relative" ref={ref}>
          <input type="text" id={name} name={name} value={value} onChange={onChange} onFocus={onFocus} required
          ...
          <div className="absolute right-4 top-1/2 transform -translate-y-1/2 pointer-events-none">
            <svg className={`w-5 h-5 text-orange-600 transition-transform duration-200 ${state.show ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {state.show && (
            <div className="absolute z-50 w-full mt-1 bg-white border-2 border-orange-400 rounded-lg shadow-lg max-h-60 overflow-y-auto">
              {state.filtered.map((item, index) => (
                <div key={index} className="px-4 py-3 hover:bg-orange-100 cursor-pointer border-b border-gray-100 last:border-b-0 transition-colors text-gray-800 font-medium text-base"
                  onClick={() => selectOption(name, item)}>{item}</div>
              ))}
            </div>
          )}
        </div>
            ...
            <div className="bg-orange-500 rounded-2xl p-6 shadow-2xl overflow-y-auto">
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <AutocompleteField
                  ...
                  onFocus={() => handleFocus('institucion')}
                ...
                <AutocompleteField
                  ...
                  onChange={handleChange}
                  onFocus={() => handleFocus('tipoPromocion')}
                  ref={refs.tipoPromocion}
                />
                ...
                <AutocompleteField
                  ...
                  onChange={handleChange}
                  onFocus={() => handleFocus('disciplina')}
                  ref={refs.disciplina}
                />

                <TextAreaField name="beneficios" label="BENEFICIOS" placeholder="Descripción de la promoción" value={formData.beneficios} onChange={handleChange} onBlur={handleBlurNormalize} limit={LIMITS.beneficios} />

                <TextAreaField name="comentariosRestricciones" label="COMENTARIOS O RESTRICCIONES" placeholder="Describe las limitantes de tu promoción" value={formData.comentariosRestricciones} onChange={handleChange} onBlur={handleBlurNormalize} limit={LIMITS.comentarios} />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <FileField name="imagenPrincipal" label="IMAGEN PRINCIPAL" value={formData.imagenPrincipal} onChange={handleChange} accept="image/*" required={true} description="Esta imagen se mostrará en el carrusel principal" />
                  <FileField name="imagenSecundaria" label="IMAGEN SECUNDARIA" value={formData.imagenSecundaria} onChange={handleChange} accept="image/*" description="Imagen adicional para la promoción (opcional)" />
                </div>
