...
// Verifica si una fecha está dentro de un rango [inicio, fin] usando cadenas ISO (evita problemas de husos horarios)
const fechaEnRango = (valorFecha, inicioISO, finISO) => {
  const fechaISO = extraerFechaISO(valorFecha);
  if (!fechaISO) return false;
  if (inicioISO && fechaISO < inicioISO) return false;
  if (finISO && fechaISO > finISO) return false;
  return true;
};

// Formatea una fecha ISO (YYYY-MM-DD) a DD/MM/YYYY para mostrar
const formatearFechaCorta = (valor) => {
  const iso = extraerFechaISO(valor) || extraerFechaISO(String(valor || '').replace(/\//g, '-'));
  if (!iso) return valor || '-';
...
const ReportesPage = () => {
  ...
  const generarReporteRedenciones = () => {
    ...
    if (fechaInicioRedenciones && fechaFinRedenciones) {
      // Comparar como cadenas ISO (YYYY-MM-DD)
      if (fechaInicioRedenciones > fechaFinRedenciones) {
        alert('La fecha de inicio no puede ser posterior a la fecha de fin');
        return;
      }
    ...
    const filtradas = redenciones.filter(redencion => {
      ...
      if (institucionSeleccionada) {
        if (normalizar(redencion.institucion) !== normalizar(institucionSeleccionada)) {
          return false;
        }
      }
  ...
  const institucionesUnicas = (() => {
    ...
    redenciones.forEach(r => {
      ...
      const clave = normalizar(r.institucion);
  ...
  const exportarRedenciones = async () => {
    ...
    try {
      ...
      const XLSX = await loadXLSX();
      const headers = ['Número de Tarjeta', 'Institución', 'Fecha', 'Tipo Promoción', 'Estado', 'ID Interno'];
      const rows = redencionesFiltradas.map(r => [
        r.numero_tarjeta || r.numeroTarjeta || '',
        r.institucion || '',
        r.fecha || r.fecha_redencion || r.created_at || r.fecha_registro || '',
                ...
                {reporteUsuariosGenerado && (
                  <div style={{ 
                    ...
                    <p style={{ color: '#3b82f6', fontSize: '13px', marginTop: '4px' }}>
                      Período: {formatearFechaCorta(fechaInicioUsuarios)} - {formatearFechaCorta(fechaFinUsuarios)}
                    </p>
                  </div>
                )}
                      ...
                      <tbody>
                        {usuariosFiltrados.map((u, idx) => (
                          <tr key={u.id} style={{ backgroundColor: idx % 2 === 0 ? 'white' : '#f9fafb', borderBottom: '1px solid #e5e7eb' }}>
                            <td style={{ padding: '12px 16px' }}>{u.numero_tarjeta || '-'}</td>
                            ...
                            <td style={{ padding: '12px 16px' }}>
                              {u.fecha_registro ? formatearFechaCorta(u.fecha_registro) : '-'}
                            </td>
                          </tr>
                        ))}
                      </tbody>
                      ...
                      <tbody>
                        {redencionesFiltradas.map((r, idx) => (
                          <tr key={r.id || idx} style={{ backgroundColor: idx % 2 === 0 ? 'white' : '#f9fafb', borderBottom: '1px solid #e5e7eb' }}>
                            ...
                            <td style={{ padding: '12px 16px' }}>
                              {(r.fecha || r.fecha_redencion || r.created_at || r.fecha_registro) 
                                ? formatearFechaCorta(r.fecha || r.fecha_redencion || r.created_at || r.fecha_registro) 
                                : '-'}
                            </td>
